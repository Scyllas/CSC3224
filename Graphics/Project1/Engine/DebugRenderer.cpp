...
namespace Engine {
	...
	void Engine::DebugRenderer::drawLine(const glm::vec2& a, const glm::vec2& b, const ColorRGBA8& color) {
		int i = m_verts.size();
	...
	void DebugRenderer::drawCircle(const glm::vec2 & center, const ColorRGBA8 color, float radius) {
		...
		for (int i = 0; i < NUM_VERTS; i++) {
			...
			float angle = ((float)i / NUM_VERTS) * PI * 2.f;
		...
		//set uup indeces for indexed drawing
