...
void EditorScreen::drawUI() {
	...
	if (m_selectMode == SelectionMode::PLACE && !isMouseInUI()) {
		...
		if (m_objectMode == ObjectMode::PLATFORM) {
			m_debugRenderer.drawBox(glm::vec4(pos - m_boxDims * 0.5f, m_boxDims), Engine::ColorRGBA8(255, 255, 255, 200), m_rotation);
			m_debugRenderer.end();
			...
			tmpLight.color = Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, (GLubyte)m_colorPickerAlpha);
			...
			// Selection radius
			m_debugRenderer.drawCircle(pos, Engine::ColorRGBA8(255, 255, 255, 255), LIGHT_SELECT_RADIUS);
			// Outer radius
		...
		if (m_selectedLight != NO_LIGHT) {
			// Selection radius
			m_debugRenderer.drawCircle(m_lights[m_selectedLight].position, Engine::ColorRGBA8(255, 255, 0, 255), LIGHT_SELECT_RADIUS);
			// Outer radius
		...
		if (m_selectedBox != NO_BOX) {
			...
			m_debugRenderer.drawBox(destRect, Engine::ColorRGBA8(255, 255, 0, 255), b.getBody()->GetAngle());
			m_debugRenderer.end();
			m_debugRenderer.render(m_camera.getCameraMatrix(), 2.0f);
		}
	}
	...
	{ // Draw the color picker quad
		...
		m_spriteBatch.draw(destRect, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), m_blankTexture.id, 0.0f, Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, 255));
		...
		m_spriteFont.draw(m_spriteBatch, colorText.c_str(), glm::vec2(destRect.x + destRect.z * 0.5f, destRect.y + destRect.w), glm::vec2(0.55f), 0.0f, Engine::ColorRGBA8(255, 255, 255, 255), Engine::Justification::MIDDLE);
	}
...
void EditorScreen::drawWorld() {
	...
	if (m_debugRender) {
		...
		for (auto& b : m_boxes) {
			...
			if (b.isDynamic()) {
				m_debugRenderer.drawBox(destRect, Engine::ColorRGBA8(0, 255, 0, 255), b.getBody()->GetAngle());
			}
			else {
				m_debugRenderer.drawBox(destRect, Engine::ColorRGBA8(255, 0, 0, 255), b.getBody()->GetAngle());
			}
		}
		// Render magenta light selection radius
		for (auto& l : m_lights) {
			m_debugRenderer.drawCircle(l.position, Engine::ColorRGBA8(255, 0, 255, 255), LIGHT_SELECT_RADIUS);
		}
		// Draw axis
		// +X axis
		m_debugRenderer.drawLine(glm::vec2(0.0f), glm::vec2(100000.0f, 0.0f), Engine::ColorRGBA8(255, 0, 0, 200));
		// -X axis
		m_debugRenderer.drawLine(glm::vec2(0.0f), glm::vec2(-100000.0f, 0.0f), Engine::ColorRGBA8(255, 0, 0, 100));
		// +Y axis
		m_debugRenderer.drawLine(glm::vec2(0.0f), glm::vec2(0.0f, 100000.0f), Engine::ColorRGBA8(0, 255, 0, 200));
		// -Y axis
		m_debugRenderer.drawLine(glm::vec2(0.0f), glm::vec2(0.0f, -100000.0f), Engine::ColorRGBA8(0, 255, 0, 100));
...
void EditorScreen::checkInput() {
	...
	while (SDL_PollEvent(&evnt)) {
		...
		case SDL_MOUSEMOTION:
			...
			break;
		case SDL_MOUSEWHEEL:
			// Dynamic for mouse wheel zoom so we multiply by getScale() instead.
...
void EditorScreen::updateMouseDown(const SDL_Event& evnt) {
	...
	static Engine::GLTexture texture = Engine::ResourceManager::getTexture("Assets/bricks_top.png");
	glm::vec2 pos;
	glm::vec4 uvRect;
	...
	case SDL_BUTTON_LEFT:
		...
		if (m_selectMode == SelectionMode::SELECT) {
			...
			case ObjectMode::PLATFORM:
				...
				newBox.init(m_world.get(), pos, m_boxDims, texture, Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, 255),
			...
			case ObjectMode::PLAYER:
				...
				// Re-init the player
				pos = m_camera.convertScreenToWorld(glm::vec2(evnt.button.x, evnt.button.y));
				m_player.init(m_world.get(), pos, glm::vec2(2.0f), glm::vec2(1.0f, 1.8f), Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, 255));
				m_hasPlayer = true;
				break;
			case ObjectMode::LIGHT:
				...
				newLight.color = Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, (GLubyte)m_colorPickerAlpha);
...
void EditorScreen::refreshSelectedBox(const glm::vec2& newPosition) {
	...
	newBox.init(m_world.get(), newPosition, m_boxDims, texture, Engine::ColorRGBA8((GLubyte)m_colorPickerRed, (GLubyte)m_colorPickerGreen, (GLubyte)m_colorPickerBlue, 255),
		false, m_physicsMode == PhysicsMode::DYNAMIC, m_rotation, uvRect);
...
bool EditorScreen::onSaveMouseClick(const CEGUI::EventArgs& e) {
	...
	Engine::IOManager::makeDirectory("Levels");
	...
	std::vector<Engine::DirEntry> entries;
	Engine::IOManager::getDirectoryEntries("Levels", entries);
...
void WidgetLabel::draw(Engine::SpriteBatch& sb, Engine::SpriteFont& sf, Engine::Window* w) {
	...
	sf.draw(sb, text.c_str(), pos, glm::vec2(scale), 0.0f, color, Engine::Justification::MIDDLE);
}
