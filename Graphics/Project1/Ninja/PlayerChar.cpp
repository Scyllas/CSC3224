...
void PlayerChar::destroy(b2World* world) {
...
void PlayerChar::draw(Engine::SpriteBatch& spriteBatch) {
	...
	destRect.x = body->GetPosition().x - m_drawDims.x / 2.f;
	destRect.y = body->GetPosition().y - m_capsule.getDimensions().y / 2.f;
	...
	for (b2ContactEdge* ce = body->GetContactList(); ce != nullptr; ce = ce->next) {
		...
		if (c->IsTouching()) {
			...
			for (int i = 0; i < b2_maxManifoldPoints; i++) {
				if (manifold.points[i].y < body->GetPosition().y - m_capsule.getDimensions().y / 2.0f + m_capsule.getDimensions().x + 0.01f) {
					below = true;
					break;
				}
			}
			if (below) {
				...
				if (inputManager.isKeyPressed(SDLK_w)) {
					body->ApplyLinearImpulse(b2Vec2(0.f, 21.f), b2Vec2(0.f, 0.f), true);
					break;
				}
			}
		}
